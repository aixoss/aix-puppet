...
(status,install/uninstall,update,reboot).'
  feature :patchmngt, 'The ability to manage actions on remote targets.', methods: [:patchmngt]
  ...
  # #####################################################################################
  newparam(:action) do
    desc '"action" parameter: simple action to perform on target : either "status", \
"update", "install", or "reboot"'
    defaultto :status
  ...
  # #####################################################################################
  newparam(:sync) do
    desc '"sync" parameter: synchronous if "yes"" or asynchronous if "no", useful \
only for "action=update"'
  ...
  # #####################################################################################
  newparam(:mode) do
    desc '"mode" parameter: update mode either "update", or "apply", or "reject", \
or "commit"" useful only for "action=update"'
    defaultto :update
